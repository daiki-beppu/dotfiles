...
| [012](012-ci-local-verification.md) | 検証を scripts/check.sh に一本化、shellcheck 自動検出化 | P2 | M | 007 | DONE（branch `feat/ci-local-verification`、commit `f46b32a` + runner 変更 `ead5c32`。レビュー済み・全 done criteria をレビュアーが再実行で確認。shebang 検出 14 本すべて severity=error pass、除外リスト空。マージ・push は未実施。Step 4 の ubuntu runner 化は CI 実走未検証 — PR 時に赤なら `ead5c32` のみ revert） |
