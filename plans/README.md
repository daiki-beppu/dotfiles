...
| [014](014-flake-cleanup.md) | flake.nix のコメントドリフトと未使用 tap を掃除 | P3 | S | — | DONE（commit `78356ab`、branch `chore/flake-cleanup`。zsh-abbr は genuinely unfree (cc-by-nc-sa-40 / hl3) — predicate に残しコメント更新。libsql/sqld tap 除去。両ホスト eval pass） |
| [015](015-review-lite-workflow.md) | review-lite workflow でレビュー run を約 -70%（spike） | P2 | M | 011 | DONE（commit `23c3575`、worktree `agent-aa1335330b8973189`。試運転未実施 — 初回実運用に委ねる。`{previous_response}` interpolation は要確認） |
