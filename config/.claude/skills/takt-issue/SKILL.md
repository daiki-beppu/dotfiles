...
複数 issue を同時に takt で回す場合は、**呼び出し元のメインペインを下に分割**して各 takt セッションを起動する。1 つ目は現在のサーフェスから `down` で分割、2 つ目以降は直前のサーフェスからさらに `down` で積み重ねる（横ではなく縦に積む）。

```bash
# 1 つ目: 現在のメインペインから下に分割
SURFACE1=$(cmux new-split down 2>&1 | awk '{print $2}')
cmux send --surface "$SURFACE1" "takt add '#<N1>'\n"
# 以降、2-A と同じ 6 段階の対話プロンプトを 1 つずつ送る

# 2 つ目以降: 直前のサーフェスからさらに下に分割
...
- 並列駆動時はメインペインを `cmux new-split down` で下に分割して takt セッションを起動し、2 つ目以降は直前のサーフェスからさらに `down` で積み重ねる
