...
- **`default`**（builtin。plan → write_tests → draft → peer-review の 4 ステップ。draft / peer-review は subworkflow で内包）: **新規 feature かつテスト先行が価値を持つ場合のみ**
- **`lite`**（カスタム、dotfiles 管理で全プロジェクト共通。plan → implement → review の 3 ステップ。structured_output でレビュー判定を決定論化し、自己監査 8 項目 policy を注入）: それ以外すべて — bugfix / chore / docs / refactor / 既存テストで担保できる変更。**迷ったらこちら**（トークン節約優先）。builtin `default-mini` の代替（default-mini は運用から外した）
- **`fix`**（カスタム、dotfiles 管理で全プロジェクト共通。fix → supervise → (必要なら) fix_supervisor → supervise の最大 3 ステップ。plan・テスト先行・並列レビューを持たない最軽量フロー）: 原因特定済みの小さなバグ修正・軽微な指摘対応で、plan / テスト先行が過剰な場合
...
- **workflow**: `default` / `lite` / `fix` のどれで回すか（下表で判定）
PR 作成は takt CLI 本体の `postExecutionFlow` が workflow 完了後に自動実行する（auto-commit → push → `gh pr create`、既存 PR があれば `gh pr comment` で追記）。`takt add` の `Auto-create PR? [Y/n]` プロンプトは **Y を選ぶ**（PR を自動で作るため）。workflow 側の品質チェックは builtin の peer-review subworkflow に含まれる reviewers（arch / ai-antipattern / pure-review / coding-review / supervise の 5 並列）が担当する（`fix` workflow は代わりに単一 supervisor 検証を使う）。
...
| 原因特定済みの小さなバグ修正・軽微な指摘対応で、plan / テスト先行が過剰 | `fix`（最軽量。plan・テスト先行・並列レビューなし） |
| 上記以外すべて — bugfix / chore / docs / refactor / 既存テストで担保できる変更 | `lite` |
| 判断に迷う場合 | `lite`（3 step 固定 + structured_output による決定論的な状態判定で 1 run あたり大幅に軽い。トークン節約を優先） |
| セキュリティ・認証系 / スキル横断の変更 | `lite` の軽量レビューでは不足するため `default` |

`default` / `lite` / `fix` のいずれも **スコープ外発見の自動 issue 起票機能を持たない**。スコープ外を見つけたときは Step 7 の人手手順（`issue` スキルへの引き渡し）で対応する旨をユーザーに伝える。
...
`lite` で回す判断なら、該当 task の `workflow: default` を `workflow: lite` に変更してから `takt run` する。既存 completed task の workflow は触らず、今回追加した `status: pending` の task だけを対象にする。

確認ポイント:

- `issue: <N>`
- `status: pending`
- `workflow: default` / `lite` / `fix`
- `auto_pr: true`
...
- `failed` / `aborted` → `tail -80 /tmp/takt_<slug>.log` で原因を読み、`.takt/runs/<run_slug>/reports/` のレポートを確認
...
2. PR URL と `.takt/runs/<run_slug>/reports/` 配下のレビューレポート（builtin の peer-review が出力する `architect-review.md` / `ai-antipattern-review.md` / `pure-review.md` / `coding-review.md` / `supervisor-validation.md` / `summary.md`）を Read で読んでレビュー結果をユーザーに表示
...
`default` / `lite` / `fix` いずれの workflow にも **スコープ外発見の自動 issue 起票機能はない**（スコープ外を自動で `gh issue create` する step は含まれない）。スコープ外を見つけたら本セクションの人手手順（`issue` スキルへの引き渡し）で必ず別 issue に切り出す。`lite` の review はスコープ外の改善提案を feedback 末尾に follow-up 候補として残すので、そこも起票の材料にする。
...
- **スコープ外自動起票は無い**: `default` / `lite` / `fix` はいずれも spillover step を持たない。スコープ外発見は必ず Step 7 の人手手順で `issue` スキルに引き渡す
...
- workflow（`default` / `lite` / `fix`）を起動前に判断・確認する。判断軸は label / 影響範囲 / テスト先行の要否。原因特定済みの小さな修正は `fix`、bugfix / chore / docs / 小規模 refactor は `lite`、feature / 中〜大規模・セキュリティ / 認証系・スキル横断は `default`、迷ったら `lite`（トークン節約優先）。いずれを選んでも spillover 自動起票がないので Step 7 の人手チェックは必須
...
- レビュー結果の所在は workflow で異なる。`default` は `.takt/runs/<run_slug>/reports/` 配下のレポート（peer-review が出力する `architect-review.md` / `ai-antipattern-review.md` / `pure-review.md` / `coding-review.md` / `supervisor-validation.md` / `summary.md`）、`lite` は review step の structured_output（verdict / feedback。run ログに記録され、レポートファイルは出力されない）、`fix` は `supervisor-validation.md` / `summary.md` のみ（並列レビューなし）。完了後は入手できる方を読んでユーザーに表示する。PR URL は `tasks.yaml` の `pr_url` または `gh pr list --head takt/<N>/<slug>` で取得する
...
- 現 issue のスコープ外の問題を見つけても worktree 内で直接修正しない。`default` / `lite` / `fix` には spillover 自動起票がないので、スコープ外発見は必ず `issue` スキルで別 issue として起票し、次回の takt サイクルに回す（判断基準: 「PR タイトルが変わるか?」変わるならスコープ外）
