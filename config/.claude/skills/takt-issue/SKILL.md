...
- **workflow**: `feature` / `improve` / `diagnose-fix` / `docs` / `lite` / `fix` のどれで回すか（下表で判定）
...
グローバル config では `planner` / `diagnoser` / `coder` persona を Codex（gpt-5）、その他（`supervisor` / reviewer 系）を Claude に振るハイブリッド構成。ただしカスタム workflow（`feature` / `improve` / `diagnose-fix` / `docs` / `lite` / `fix`）は全 step で `provider: codex` を明示しており、review も Codex で走る。詳細と rate limit リカバリ手順は `~/.claude/skills/takt/SKILL.md` の `persona_providers` セクションを参照。
...
非 TTY では意図しない workflow（一覧の先頭）が選ばれることがあるため、該当 task の `workflow` を Step 1 で判定したもの（`feature` / `improve` / `diagnose-fix` / `docs` / `lite` / `fix`）に変更してから `takt run` する。既存 completed task の workflow は触らず、今回追加した `status: pending` の task だけを対象にする。

確認ポイント:

- `issue: <N>`
- `status: pending`
- `workflow: feature` / `improve` / `diagnose-fix` / `docs` / `lite` / `fix`
- `auto_pr: true`
...
- `failed` / `aborted` → `tail -80 /tmp/takt_<slug>.log` で原因を読み、`.takt/runs/<run_slug>/reports/` のレポートを確認。**ただし以下 2 つは設計上の正常な停止**（エラーではない）なので、失敗として扱わず専用ハンドリングに進む:
  ...
  - **`feature` の scope_review 停止**: implement ⇄ review が空転してスコープ見直しに切り替わった。`.takt/runs/<run_slug>/reports/scope-review.md` を読み、分割提案を**ユーザーに提示**して `to-issues` / `issue` スキルで slice ごとに起票する（worktree の部分実装は分割 issue の参照資料としてパスを記録してからクリーンアップする）
...
- **ABORT が正常系の workflow がある**: `diagnose-fix` の診断停止（自動修正条件未達）と `feature` の scope_review 停止（空転検知）は設計どおりの停止。失敗として retry せず、Step 4「完了時の確認」の専用ハンドリング（診断レポートのコメント / 分割提案の提示）に進む
...
- workflow（`feature` / `improve` / `diagnose-fix` / `docs` / `lite` / `fix`）を起動前に判断・確認する。判断軸は「新規 or 既存挙動の変更 or バグ / 原因は特定済みか / コード変更を伴うか / interface 変更を伴うか」。新規 feature・セキュリティ / 認証系・interface 変更は `feature`、既存機能の挙動変更は `improve`、原因不明バグは `diagnose-fix`、原因特定済みの小さな修正は `fix`、ドキュメントのみは `docs`、refactor / chore は `lite`、迷ったら `lite`（トークン節約優先）。いずれを選んでも spillover 自動起票がないので Step 7 の人手チェックは必須
...
- レビュー結果の所在は workflow で異なる。`lite` / `improve` / `docs` / `feature` は review step の structured_output（verdict / feedback / followups。run ログに記録される）、`diagnose-fix` は diagnose の structured_output（report_md / followups）と supervise の structured_output、`fix` は `supervisor-validation.md` / `summary.md`。`feature` は加えて `plan.md` / `test-design.md`（空転停止時は `scope-review.md`）が `.takt/runs/<run_slug>/reports/` に出力される。完了後は入手できる方を読んでユーザーに表示する。PR URL は `tasks.yaml` の `pr_url` または `gh pr list --head takt/<N>/<slug>` で取得する
