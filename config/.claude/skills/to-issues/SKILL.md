...
- 特権型の品質ゲートは、各 issue の `## 品質ゲート` に「CI で exit 0 になる」形式で記載し、agent のローカル検証対象にしない
...
（コード・設定変更では必須。リポジトリから特定した lint / test / typecheck コマンドを記載する。`## 要件`（= 受入条件、functional）とは別の mechanical なゲート。特権型またはローカルの必要ツールチェーンを確保できないゲートは `CI で exit 0 になる` 形式で記載し、agent のローカル検証対象にしない。docs のみの変更で該当するゲートがない場合は理由を明記する）
...
- **コード・設定変更では `## 品質ゲート` を必須とし、リポジトリから特定した lint / test / typecheck のうち該当するコマンドを記載する**（`## 要件`（functional）とは別の mechanical なゲート）。起票時には実行せず、実装完了時に成功することを受入条件にする。特権型またはローカルの必要ツールチェーンを確保できないゲートは「CI で exit 0 になる」形式で記載し、agent のローカル検証対象にしない。コマンドが存在しない・特定できない場合は推測せず理由を明記する
