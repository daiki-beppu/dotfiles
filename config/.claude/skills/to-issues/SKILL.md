...
- workflow: lite / default / 不要（手動実装が妥当）
- 理由: ...
```
...
- **`## 実装方針（takt）`**: スライスの特徴から `default` / `lite` / `不要` を判断。bugfix / chore / 少ファイル → `lite`、feature / テスト先行が価値を持つ・セキュリティ / 認証系 → `default`、1 行修正 → `不要`。迷ったら `lite`
