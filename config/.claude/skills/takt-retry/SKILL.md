...
| agent（Claude Code 等）から復帰したい | 下記「agent からの復帰」（非対話の破棄 → 再 add、もしくはユーザーに対話コマンドの実行を依頼） |
...
`takt list --non-interactive` の `--action` は `diff|try|merge|delete` のみで **retry / requeue は非対話にない**。agent から復帰する場合は次の 2 択:

1. **破棄 → 再 add**（非対話で完結。既存 worktree の部分実装は引き継がない）
2. **ユーザーに対話コマンドの実行を依頼**（既存 worktree・resume point を活かしたいとき。実行すべきコマンド `takt list` / `takt resume` と選ぶべきアクション・追加指示の案を提示して、ユーザーに委ねる）

追加指示だけで直りそうな失敗や、失敗 step 途中からの再開が有効な失敗は 2 が向く。環境要因やゼロからやり直したほうが早い失敗は 1 でよい。
