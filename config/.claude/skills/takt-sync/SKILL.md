...
description: >-
  ...
  --check で調査レポートだけ、--apply で適用から、--version / --assets-only / --doctor / --registry で範囲を絞れる。
---
...
- Bare invocation → フェーズ 1（調査）→ レポート提示 → 適用範囲の合意 → フェーズ 2（適用）を通す。
- `--check` → フェーズ 1 だけ実行してレポートを出し、**そこで止まる**。フェーズ 2 には進まない。
- `--apply` → 適用範囲の合意が既にある前提でフェーズ 2 から始める。
- `--version <vX.Y.Z>` → 追随先バージョンを明示する（既定は最新タグ。フェーズ 1 の 1）。
- `--assets-only` → 本体更新（フェーズ 2 の 1）をスキップし、資産の移行と検証だけ行う。
- `--doctor` → フェーズ 1 の 4（検証ベースライン取得）だけを実行し、既存エラーの一覧を報告する。
- `--registry` → レジストリの鮮度確認（フェーズ 1 の 3 冒頭）だけを実行する。
...
`--apply` で直接ここへ入る場合も、適用範囲の合意（同一セッション内の調査レポート、または
直前の `--check` 結果）が前提になる。合意の記録が見当たらなければ着手せず、
`--check` を先に回すよう促す。`--assets-only` は既に `darwin-rebuild switch` 済みで
プロジェクト資産だけが追いついていないときに使う。
