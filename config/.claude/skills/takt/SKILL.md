...
| `solid` | 4 + loop_monitor | 12 | lite の一段上の堅牢版。plan（失敗原因分析 + スコープゲート）→ implement（最初から gpt-5.6-sol）→ review。lite で完了できなかった task の再走用（まっさらな worktree でゼロから再実装）。スコープ過大・空転時は scope_review が分割案を出して停止 |
