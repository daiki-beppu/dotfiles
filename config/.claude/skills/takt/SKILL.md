...
- bugfix / chore / docs / refactor / 既存テストで担保できる変更は `lite`。迷った場合も `lite`。
- 新規 feature、テスト先行が設計に効く変更、認証・セキュリティ・スキル横断の変更は `default`。
...
dotfiles 環境は **eject なし**（builtin の `default` はそのまま使用）。カスタム資産は eject ではなく新規作成で持つ: workflow `lite` / `e2e-verify` / `fix`、policy `pre-review-checklist`、schema `review-verdict`（いずれも dotfiles の `config/.takt/` 配下で git 管理、`~/.takt/` への symlink 経由で解決）。builtin を改造したくなったら以下に従って eject する。
