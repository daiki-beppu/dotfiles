...
|----------|---------|-----------|------|
| `default-extended` | 13 | 60 | テスト先行開発。計画 → 計画レビュー → テスト実装 → 実装 → AI レビュー → 並列レビュー → spillover 起票 |
| `default-mini` | 6 | 30 | テスト省略の軽量版。計画 → 実装 → AI レビュー → 並列レビュー |

両者の違いは **テスト実装フェーズの有無** と **`report_spillover` step の有無**。
...
- `config/.takt/facets/instructions/finalize-pr.md` /
