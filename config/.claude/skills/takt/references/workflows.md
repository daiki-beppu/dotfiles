...
運用 workflow（builtin `default` + カスタム `lite` / `fix`）の step 構成と subworkflow 内訳。SKILL.md 本文の [Workflow](../SKILL.md#workflow) 節から参照される。

dotfiles 環境は **eject なし**（builtin の `default` はそのまま使用）。カスタム資産は新規作成で持つ: workflow `lite`（builtin `default-mini` の代替）/ `e2e-verify` / `fix`、policy `pre-review-checklist`、schema `review-verdict`。実体は dotfiles の `config/.takt/` 配下（git 管理）、`~/.takt/workflows/` 等への symlink 経由で全プロジェクトから解決される。
