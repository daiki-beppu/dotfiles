...
| 名前 | 用途 |
|------|------|
| `loop-monitor-ai-fix` | ai_review ↔ ai_fix ループの判定 |
...
| 名前 | 用途 |
|------|------|
| `report-scope-spillover` | スコープ外発見の自動起票（`[user]` でカスタマイズ済み、`gh issue create` 連携） |
...
| `ai-review` | AI レビュー結果 (`ai-review.md`) |
...
takt eject default-extended                # → .takt/workflows/default-extended.yaml
