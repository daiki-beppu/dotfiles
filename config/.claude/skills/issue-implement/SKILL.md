...
- **集合外の blocker**: state でなく **PR の有無**で判定する — `gh pr list --search "<B> in:body" --state open`。PR があれば(OPEN / MERGED とも)その上に積んで着手してよい。OPEN かつ PR 無しでも、**その blocker を同じスタックの下段として段の列に加えるなら着手してよい**(積む先を自分で作るため)。段に加えられない blocker だけ一覧を提示してユーザーに確認。
