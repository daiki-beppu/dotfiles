...
6. **ラベル選定**: 以下の 3 ステップで候補を絞り込む
   ...
   **Step D: takt 適用ラベル**（Step 5 の判定結果を反映）
   - Step 5 の判定に対応する takt ラベルを 1 つ必ず候補に含める（`takt:default` / `takt:lite` / `takt:manual`）
...
7. **プレビュー表示**: 生成内容を以下のフォーマットで表示する。各ラベルには選定理由を 1 行で添える
   ...
   **重複チェック**: プレビュー生成前に、Step 1 で取得済みの過去 issue 一覧（open 中心、必要なら state 問わず）とタイトルを突き合わせ、確定したタイトル（Step 4）と語幹が近い issue が無いか確認する。該当する場合は本文の前に以下の警告ブロックを添えてプレビューする:
