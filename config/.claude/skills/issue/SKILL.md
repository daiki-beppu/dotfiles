...
3. **本文生成（カテゴリ別テンプレ + 共通コア構造）**: 以下の構造に厳格に揃えて本文を生成する。**セクション名・順序は takt の `plan.md` instruction が解釈する形に合わせるため変更しない**
   ...
   生成した `## 要件` と `## 影響ファイル` のサイズが takt の 1 run で完結するスコープを超えていないか検証する。以下のいずれかに該当する場合は **分割を提案** する:
...
- **`## 要件` は番号付きリストで検証可能な単位に分解する**（完了条件のメイン）。**要件 8 件以上・影響ファイル 10 以上・独立機能領域 2 つ以上混在の場合は sub-issue 分割を提案する**（takt の 1 run で完結するスコープに収める）
