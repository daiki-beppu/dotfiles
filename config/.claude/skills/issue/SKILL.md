...
description: |
  ...
  起票前に必ず grilling スタイルのインタビュー（1 問ずつ・推奨回答つき）で内容を合意してから作成する。
...
2. **インタビュー（grilling スタイル）**: 本文生成に入る前に、**必ず**ユーザーへのインタビューで issue の内容を合意する。会話コンテキストと Step 1 の調査結果から各決定事項の仮説を組み立て、決定の依存関係の順に 1 つずつ確認しながら設計ツリーを降りていく
   ...
   | 8 | takt 方針（Step 6 の 3 値から推奨を提示して確認） | 実装方針（takt）・ラベル |
...
8. **プレビュー表示**: 生成内容を以下のフォーマットで表示する。各ラベルには選定理由を 1 行で添える
...
9. **ユーザー確認**: 「この内容で作成しますか？修正があれば教えてください」と確認する。修正指示があれば反映してから再度プレビューする。新規ラベル候補は個別に承認/拒否を確認する

10. **issue 作成**: 承認後、必要に応じて `gh label create` を実行してから `gh issue create` で作成する

   ```bash
   gh issue create --title "タイトル" --body "本文" --label "label1,label2"
   ```

11. **URL を表示**: 作成された issue の URL を表示する
