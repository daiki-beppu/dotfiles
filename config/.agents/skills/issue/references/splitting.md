...
接続詞の数、ファイル数、要件行数だけで機械的に分割しない。一つの成果を検証するために複数の要件が必要な場合もある。独立して完了・レビューできる成果と依存から粒度を決め、`--split` / `--no-split` の指定を優先する。

分割時は全体の目的・仕様・完了条件を持つ親を再利用または作成し、[parent.md](parent.md) で全ての子を native sub-issue として接続する。既存 issue の整理全体が必要な場合だけ [organize.md](organize.md) を読む。
...
`totalBlockedBy` は件数であり、期待する相手との接続を単独では証明しない。接続後に対象の `blockedBy` をページネーション込みで取得し、依存先と方向を確認する。
...
着手可能なのは `blockedBy` が空、または必要な blocker の成果が揃った issue。close は未実装での取り下げもあり得るので、state だけで成果の存在を判定しない。
